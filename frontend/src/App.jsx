  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      <nav className="sticky top-0 z-50 border-b border-border-subtle bg-white/90 backdrop-blur-xl">
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                      isActive
                        ? 'bg-accent-green/15 text-accent-green shadow-sm shadow-accent-green/10'
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-xl hover:bg-accent-green/5 transition"
              >
                <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-accent-green to-emerald-300 flex items-center justify-center text-white text-xs font-bold">
                  {user?.initials || 'U'}
                </div>
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-1 w-56 rounded-xl border border-border-subtle bg-white/95 backdrop-blur-xl shadow-xl shadow-black/20 py-1 z-50 fade-in">
                  <div className="px-4 py-3 border-b border-border-subtle">
                    <p className="text-sm font-medium text-text-primary">{user?.name}</p>
                    <p className="text-xs text-text-muted">{user?.email}</p>
                    <p className="text-[0.6rem] text-accent-green mt-0.5">{user?.role}</p>
                  </div>
                  <NavLink
                    to="/settings"
                    onClick={() => setUserMenuOpen(false)}
                    className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-text-secondary hover:bg-accent-green/5 transition text-left"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            {/* Mobile hamburger */}
            <button
              className="md:hidden p-2 rounded-lg hover:bg-accent-green/5 text-text-secondary"
        ...
        {/* Mobile dropdown */}
        {mobileOpen && (
          <div className="md:hidden border-t border-border-subtle bg-white/95 backdrop-blur-xl">
                    ...
                    className={({ isActive }) =>
                      `flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all ${
                        isActive
                          ...
                          : 'text-text-secondary hover:bg-accent-green/5'
                      }`
                    }
                  >
